...
def write_emodb_csv(emotions=["sad", "neutral", "happy"], train_name="train_emo.csv",
    ...
    for emotion, code in categories_reversed.items():
        if emotion not in emotions:
            del categories[code]
    for file in glob.glob("/content/drive/Shareddrives/Hx-corps/AI HUB + RAVDESS 코드/github_project/emotion-recognition-using-speech/data/emodb/wav/*.wav"):
        try:
...
def write_tess_ravdess_csv(emotions=["sad", "neutral", "happy"], train_name="train_tess_ravdess.csv",
    ...
    for category in emotions:
        ...
        total_files = glob.glob(f"/content/drive/Shareddrives/Hx-corps/AI HUB + RAVDESS 코드/github_project/emotion-recognition-using-speech/data/training/Actor_*/*_{category}.wav")
        ...
        total_files = glob.glob(f"/content/drive/Shareddrives/Hx-corps/AI HUB + RAVDESS 코드/github_project/emotion-recognition-using-speech/data/validation/Actor_*/*_{category}.wav")
        for i, path in enumerate(total_files):
...
def write_custom_csv(emotions=['sad', 'neutral', 'happy'], train_name="train_custom.csv", test_name="test_custom.csv",
    ...
    for category in emotions:
        # train data
        for i, file in enumerate(glob.glob(f"/content/drive/Shareddrives/Hx-corps/AI HUB + RAVDESS 코드/github_project/emotion-recognition-using-speech/data/train-custom/*_{category}.wav")):
        ...
        # test data
        for i, file in enumerate(glob.glob(f"/content/drive/Shareddrives/Hx-corps/AI HUB + RAVDESS 코드/github_project/emotion-recognition-using-speech/data/test-custom/*_{category}.wav")):
